...
final class Store: ObservableObject {
    ...
    private let fileName = "sunexposure_entries.json"
    ...
    private func seed() {
        let now = Date()
        entries = [
            SessionEntry(date: now.addingTimeInterval(-86400*2), note: "First entry", minutes: 10, tag: "General", rating: 2),
            SessionEntry(date: now.addingTimeInterval(-86400), note: "Second entry", minutes: 15, tag: "General", rating: 3)
        ]
        save()
    }
    ...
    private func load() {
        ...
        if let decoded = try? JSONDecoder().decode([SessionEntry].self, from: data) {
            entries = decoded
        }
    }
