    ...
    private func entryRow(_ entry: SessionEntry) -> some View {
        HStack {
            VStack(alignment: .leading, spacing: 4) {
                Text(entry.tag.isEmpty ? "Session" : entry.tag)
...
struct EntryEditorView: View {
    ...
    let existing: SessionEntry?
    let onSave: (SessionEntry) -> Void

    init(entry: SessionEntry?, onSave: @escaping (SessionEntry) -> Void) {
        self.existing = entry
        self.onSave = onSave
        ...
        NavigationStack {
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .confirmationAction) {
                    Button("Save") {
                        var entry = existing ?? SessionEntry()
                        entry.tag = tag
                        entry.note = note
